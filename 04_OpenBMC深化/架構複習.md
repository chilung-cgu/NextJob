...
│    - SPI: 3 組                                                  │
│    - UART: 13 組                                                │
│    - GPIO: 208+36 個 (gpio0+gpio1，視 Device Tree 定義)                                               │
│    - PWM/TACH: 16 組                                            │
│    - ADC: 16 通道                                               │
...
# 6. 提交修改到 Layer (最重要的步驟)
# 這是將 Workspace 的暫時修改轉正為 Yocto Recipe 的關鍵指令

# ⚠️ 步驟 A：務必先 Commit
# devtool 會根據 git commit 產生 patch 檔
# 若沒有 commit，update-recipe 會失敗或產生空 patch
cd build/workspace/sources/phosphor-hwmon
git add .
git commit -m "Fix: adjust sensor threshold for Yosemite4"

# ⚠️ 步驟 B：執行 update-recipe (兩種模式)

# 模式 1：直接修改原始 Recipe (不推薦)
# 這會直接修改 meta-phosphor 裡面的 .bb 檔
# 缺點：汙染上游 Layer，導致之後與社群同步困難
# devtool update-recipe phosphor-hwmon

# 模式 2：建立 .bbappend (💡 推薦 Best Practice)
# 將修改變成 Patch 檔，並放在你指定的 Layer (如 meta-facebook)
# 格式：devtool update-recipe <recipe-name> -a <layer-path>
devtool update-recipe phosphor-hwmon -a /home/chilung/openbmc/meta-facebook

# 執行後的結果：
# 1. 在 meta-facebook/.../phosphor-hwmon/ 目錄下產生 .patch 檔案
# 2. 自動產生或更新 .bbappend 檔案，加入 SRC_URI += "file://xxx.patch"
# 3. 保持 meta-phosphor (上游) 乾淨未修改

# 7. 完成開發，清理 Workspace
# 這會移除 workspace/sources 下的暫存碼，讓 bitbake 回復使用 layer 中的設定
devtool reset phosphor-hwmon
```

### 🚀 完整實戰演練：修改 phosphor-hwmon 並提交到 meta-facebook

**情境**：
我們發現 `phosphor-hwmon` 在讀取 Sensor 失敗時 log 太少，難以 Debug。
目標是修改原始碼加入詳細 log，並將修改正確封裝到 `meta-facebook` layer。
...
# 2. 將 phosphor-hwmon 的原始碼拉到 workspace
# 系統會將原始碼解壓到 build/workspace/sources/phosphor-hwmon
devtool modify phosphor-hwmon
```
...
+        // [Modified] 加上 errno 和路徑，方便 Debug
+        log<level::ERR>("Failed to read sensor",
+                        entry("PATH=%s", _path.c_str()),
+                        entry("RC=%d", rc)); 
     }
 }
```
...
**Step 3: 提交修改 (Commit)**
⚠️ **關鍵步驟**：devtool 是根據 git commit 來製作 patch 的。
```bash
git add sensor.cpp
git commit -m "Debug: add detailed log when sensor read fails"
```

**Step 4: 回寫到 Layer**
將 commit 轉為 patch，並存入 meta-facebook 的 bbappend 中。
```bash
# -a: append (產生 bbappend)
# 指定路徑到 meta-facebook
devtool update-recipe phosphor-hwmon -a /home/chilung/openbmc/meta-facebook
```

**Step 5: 驗證結果**
檢查 `meta-facebook` 是否自動產生了對應檔案：
```text
meta-facebook/recipes-phosphor/sensors/
├── phosphor-hwmon_%.bbappend          <-- [自動更新] SRC_URI 加入 patch
└── phosphor-hwmon/                    <-- [自動建立目錄]
    └── 0001-Debug-add-detailed-log... <-- [自動產生 Patch 檔]
```

**Step 6: 收尾**
移除 workspace，回復正常 build 流程。
```bash
devtool reset phosphor-hwmon
```

### 🔄 devtool finish vs devtool update-recipe 差異

```
這兩個指令經常讓人混淆，以下說明它們的差異：

┌──────────────────────────────────────────────────────────────────────────────┐
│                  devtool update-recipe vs devtool finish                     │
├──────────────────┬───────────────────────────┬───────────────────────────────┤
│       項目       │   devtool update-recipe   │       devtool finish          │
├──────────────────┼───────────────────────────┼───────────────────────────────┤
│ 主要目的         │ 產生 patch 並更新 recipe  │ 結束開發並清理 workspace      │
│ Workspace 狀態   │ 保留（可繼續開發）        │ 清除（開發結束）              │
│ 等同於           │ 只做 patch + recipe 更新  │ update-recipe + reset         │
│ 使用時機         │ 開發中途，想保存進度      │ 開發完成，準備收尾            │
│ 可反覆執行       │ ✅ 可以多次執行           │ ❌ 執行後 workspace 被清除    │
└──────────────────┴───────────────────────────┴───────────────────────────────┘

💡 簡單記法：
   - update-recipe = 存檔（可繼續玩）
   - finish = 存檔 + 關閉遊戲
```

#### devtool update-recipe 詳解

```bash
# 語法
devtool update-recipe <recipe-name> [-a <layer-path>]

# 功能：
# 1. 將 workspace 中的 git commits 轉換為 patch 檔
# 2. 更新 recipe 或 .bbappend 的 SRC_URI
# 3. 保留 workspace，可繼續開發
...
# 第一次修改
vim sensor.cpp
git add . && git commit -m "Fix: first change"
devtool update-recipe phosphor-hwmon -a ~/openbmc/meta-facebook
# → patch 被建立，workspace 仍然存在

# 第二次修改（繼續開發）
vim sensor.cpp
git add . && git commit -m "Fix: second change"
devtool update-recipe phosphor-hwmon -a ~/openbmc/meta-facebook
# → patch 被更新（合併所有 commits），workspace 仍然存在
...
# 功能：
# 1. 執行 update-recipe 的所有動作（產生 patch、更新 recipe）
# 2. 自動執行 devtool reset 清理 workspace
# 3. 一步完成收尾
...
# 修改程式碼
vim sensor.cpp
git add . && git commit -m "Fix: complete solution"

# 一步完成（等同於 update-recipe + reset）
devtool finish phosphor-hwmon ~/openbmc/meta-facebook
# → patch 被建立
# → bbappend 被更新
# → workspace 被清除
# → 無法再繼續開發（需要重新 devtool modify）
```

#### 使用情境對比

```
情境 A：開發過程中想先保存進度（推薦 update-recipe）
─────────────────────────────────────────────────────
你正在開發一個複雜的修改，想先把目前的進度保存到 layer，
但還沒完全做完，之後還要繼續開發。

做法：
1. git commit（保存目前進度）
2. devtool update-recipe phosphor-hwmon -a ~/openbmc/meta-facebook
3. 繼續開發...
4. git commit（新進度）
5. devtool update-recipe... （再次更新）
6. devtool reset（最後才清理）

好處：
- 中途可以 bitbake 驗證 patch 是否正確
- 不影響 workspace，可繼續修改


情境 B：開發完成，準備提交（推薦 finish）
─────────────────────────────────────────────────────
你已經完成開發和測試，確定修改是正確的，
想要快速完成收尾動作。
...
做法：
1. git commit（最終版本）
2. devtool finish phosphor-hwmon ~/openbmc/meta-facebook
3. 完成！

好處：
- 一個指令完成所有動作
- 自動清理 workspace，保持環境乾淨
```

### 🚀 devtool finish 完整實戰演練

```bash
# ============================================================
# 情境：使用 devtool finish 快速完成 phosphor-hwmon 修改
# ============================================================
...
# Step 4: Commit
git add .
git commit -m "Fix: improve sensor reading error handling"

# Step 5: 一步完成收尾 (核心差異！)
#         注意：finish 的參數是 layer 路徑，不是 -a
devtool finish phosphor-hwmon ~/openbmc/meta-facebook

# 結果：
# ✅ meta-facebook/.../phosphor-hwmon/ 產生 patch
# ✅ meta-facebook/.../phosphor-hwmon_%.bbappend 被更新
# ✅ build/workspace/sources/phosphor-hwmon 被刪除
# ✅ 之後 bitbake 會使用 layer 中的 patch
...
# 錯誤 1：finish 後想繼續修改
# ─────────────────────────────
# 問題：執行 finish 後發現還有東西要改
# 解法：需要重新 devtool modify
devtool modify phosphor-hwmon
# 之前的 patch 會自動 apply，你可以繼續修改

# 錯誤 2：忘記 commit 就執行 update-recipe / finish
# ───────────────────────────────────────────────────
# 問題：沒有 commit 的修改不會被包含在 patch 中
# 解法：確保所有修改都 commit 了
git status  # 檢查是否有未 commit 的修改
git add .
git commit -m "..."
devtool update-recipe ...  # 或 devtool finish ...

# 錯誤 3：update-recipe 和 finish 參數格式不同
# ───────────────────────────────────────────────
# update-recipe 用 -a 指定 layer
devtool update-recipe phosphor-hwmon -a ~/openbmc/meta-facebook

# finish 直接指定 layer（不需要 -a）
devtool finish phosphor-hwmon ~/openbmc/meta-facebook
```
...
### 📖 深入理解：phosphor-hwmon 為什麼需要 .conf 設定檔？

```
核心問題：Kernel 只知道「原始數據」，不知道「名字」

當 Kernel 建立 /sys/class/hwmon/hwmon0/ 時，它只會顯示：

/sys/class/hwmon/hwmon0/
├── name          → "tmp75"       (driver 名稱)
├── temp1_input   → 45250         (毫度，45.25°C)
├── temp1_max     → 85000
└── temp1_crit    → 95000

問題：
❓ Kernel 不知道這顆 Sensor 是「CPU 溫度」還是「主機板溫度」
❓ D-Bus 和 Redfish 需要有意義的名稱，如 "CPU0_Temp"
❓ 閾值可能需要客製化（不同平台不同規格）
...
```bash
# 設定檔：/etc/default/obmc/hwmon/2-0048.conf
#
# 檔名解釋：
#   2      = I2C Bus 2
#   0048   = I2C 地址 0x48
...
# 解釋：
# LABEL_temp1   → 將 sysfs 的 temp1_input 對應到 D-Bus 名稱 "CPU0_Temp"
# WARNHI_temp1  → 設定警告上限閾值 75°C (milli-degree)
# CRITHI_temp1  → 設定危險上限閾值 90°C
```
...
sysfs: /sys/class/hwmon/hwmon0/temp1_input = 45250

    phosphor-hwmon: 「我不知道這是什麼 Sensor，叫什麼名字，無法發布！」
...
sysfs: /sys/class/hwmon/hwmon0/temp1_input = 45250
    ...
    ┌──────────────────────────────────────────────────┐
    │  phosphor-hwmon 讀取設定檔 2-0048.conf            │
    │                                                  │
    │  LABEL_temp1=CPU0_Temp                           │
    │     → 「哦！temp1 應該叫做 CPU0_Temp」            │
    │                                                  │
    │  WARNHI_temp1=75000                              │
    │     → 「warning 閾值是 75°C」                     │
    └──────────────────────────────────────────────────┘

D-Bus: /xyz/openbmc_project/sensors/temperature/CPU0_Temp
       ├── Value = 45.25
       └── WarningHigh = 75.0
```

#### 完整實戰範例：為 Yosemite 4 新增溫度 Sensor

```
情境：TMP75 IC 連接在 I2C Bus 5, 地址 0x4A

Step 1: Device Tree 定義（讓 Kernel 知道有這個硬體）
───────────────────────────────────────────────────
```

```dts
// meta-facebook/.../yosemite4.dts
&i2c5 {
    sensor@4a {
        compatible = "ti,tmp75";    // 這會讓 Kernel 載入 tmp75 driver
        reg = <0x4a>;
    };
};
```
...
```
Step 2: 建立設定檔（讓 phosphor-hwmon 知道怎麼命名）
───────────────────────────────────────────────────
```

```bash
# 檔案位置：meta-facebook/.../phosphor-hwmon/5-004a.conf
# 檔名 = <bus>-<address>

LABEL_temp1=MB_Inlet_Temp
WARNHI_temp1=50000
WARNLO_temp1=5000
CRITHI_temp1=60000
CRITLO_temp1=0
...
```
Step 3: 建立 .bbappend 安裝設定檔
───────────────────────────────────────────────────
```

```bash
# 檔案：meta-facebook/.../phosphor-hwmon_%.bbappend

FILESEXTRAPATHS:prepend := "${THISDIR}/${PN}:"

SRC_URI += "file://5-004a.conf"

do_install:append() {
    install -d ${D}${sysconfdir}/default/obmc/hwmon
    install -m 0644 ${WORKDIR}/5-004a.conf \
        ${D}${sysconfdir}/default/obmc/hwmon/
}
...
1. Kernel 載入 tmp75 driver
   → /sys/class/hwmon/hwmon1/temp1_input = 32500

2. phosphor-hwmon 讀取 5-004a.conf
   → 發現 LABEL_temp1=MB_Inlet_Temp

3. D-Bus 出現：
   /xyz/openbmc_project/sensors/temperature/MB_Inlet_Temp
   ├── Value = 32.5
   ├── WarningHigh = 50.0
   └── CriticalHigh = 60.0

4. Redfish 可查詢：
   GET /redfish/v1/Chassis/chassis/Sensors/MB_Inlet_Temp
   → {"Reading": 32.5, "ReadingUnits": "Cel"}
```

### 🔬 重要概念釐清：Device Tree、Driver、phosphor-hwmon 的關係

```
❓ 常見疑問解答（經驗證的正確說明）

Q1: Device Tree 定義 driver，是不是一開機就會 load driver，不管 sensor 硬體是否存在？

A1: ✅ 是的！這正是使用 Device Tree 的行為特性。
...
│                                                                             │
│  開機時：                                                                    │
│  1. Kernel 解析 DTB (Device Tree Blob)                                      │
│  2. 發現 compatible = "ti,tmp75" 的節點                                     │
│  3. 尋找並載入對應的 driver (drivers/hwmon/lm75.c)                          │
│  4. 呼叫 driver 的 probe() 函數                                             │
│                                                                             │
│  如果硬體存在：                                                              │
│  → probe() 成功，建立 /sys/class/hwmon/hwmonX/                              │
│  → 一切正常                                                                  │
│                                                                             │
│  如果硬體不存在：                                                            │
│  → probe() 失敗，I2C 通訊超時                                               │
│  → dmesg 出現錯誤訊息：                                                      │
│      [   15.234567] tmp75 2-0048: Failed to read device id                  │
│      [   15.234890] tmp75 2-0048: probe failed                              │
│  → sysfs 沒有建立                                                           │
│                                                                             │
│  ⚠️ 這就是為什麼 Device Tree 不適合 Slot 可插拔的情境                       │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘


Q2: Device Tree 有定義 compatible，就是 phosphor-hwmon？就是開機會 load driver？
...
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  Device Tree 的 compatible = "ti,tmp75" 做的事：                            │
│  ─────────────────────────────────────────────────────────────              │
│  1. 告訴 Kernel：「這個位置有個硬體，請用 tmp75 driver 來處理」              │
│  2. Kernel 載入 driver，呼叫 probe()                                        │
│  3. 如果成功，建立 /sys/class/hwmon/hwmonX/ 的 sysfs 介面                   │
│                                                                             │
│  ❓ 這跟 phosphor-hwmon 有什麼關係？                                        │
│  ─────────────────────────────────────────────────────────────              │
│  完全無關！                                                                  │
│                                                                             │
│  - compatible 是 Kernel 層面的概念                                          │
│  - phosphor-hwmon 是 OpenBMC User Space 的應用程式                          │
│  - phosphor-hwmon 只是「讀取」sysfs，不參與 driver 載入                     │
│                                                                             │
│  正確的理解：                                                                │
│  ┌────────────────┐         ┌────────────────┐         ┌────────────────┐   │
│  │  Device Tree   │ ──→     │  Kernel        │ ──→     │  sysfs         │   │
│  │  compatible    │         │  載入 driver   │         │  /sys/class/   │   │
│  └────────────────┘         └────────────────┘         │  hwmon/        │   │
│                                                        └───────┬────────┘   │
│                                                                │             │
│                                                                ▼             │
...
Q3: dbus-sensors 會直接依賴 Device Tree 定義的 driver 嗎？

A3: 不一定！這是 dbus-sensors 與 phosphor-hwmon 的關鍵差異。

┌─────────────────────────────────────────────────────────────────────────────┐
│  dbus-sensors 的兩種運作模式                                                 │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  模式 A：配合 Device Tree（與 phosphor-hwmon 類似）                         │
│  ─────────────────────────────────────────────────────────                  │
│  - Device Tree 定義硬體 → Kernel 載入 driver → 建立 sysfs                   │
│  - entity-manager JSON 設定 Probe 條件                                      │
│  - dbus-sensors 讀取 sysfs                                                  │
│                                                                             │
│  模式 B：Runtime 動態實例化（現代推薦方式）❌ 不依賴 Device Tree            │
│  ─────────────────────────────────────────────────────────                  │
│  - Device Tree 只定義 I2C Controller（不定義 Sensor）                       │
│  - entity-manager 在 runtime 掃描 I2C Bus                                   │
│  - 發現硬體後，透過 sysfs 的 new_device 動態實例化 driver：                 │
│      echo "tmp75 0x48" > /sys/bus/i2c/devices/i2c-2/new_device              │
│  - Kernel 動態載入 driver，建立 sysfs                                       │
│  - dbus-sensors 讀取 sysfs                                                  │
│                                                                             │
│  💡 這就是為什麼 dbus-sensors + entity-manager 可以處理 Slot 插拔           │
│     因為 driver 是「發現硬體後才載入」，不是「開機就載入」                   │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
```
┌──────────────────────────────────────────────────────────────────────────────┐
│                    I2C Driver 載入的兩種方式對比                              │
├───────────────────┬────────────────────────────┬─────────────────────────────┤
│       項目        │    Device Tree (靜態)      │   new_device (動態)          │
├───────────────────┼────────────────────────────┼─────────────────────────────┤
│ 定義時機          │ 編譯時 (.dts 檔案)         │ 運行時 (entity-manager)      │
│ Driver 載入時機   │ 開機時                     │ 偵測到硬體後                  │
│ 硬體不存在時      │ probe 失敗，dmesg 報錯     │ 不載入，無錯誤                │
│ 適用情境          │ 硬體固定不變               │ Slot 可插拔、動態硬體         │
│ OpenBMC 搭配      │ phosphor-hwmon 或          │ dbus-sensors +               │
│                   │ dbus-sensors               │ entity-manager               │
│ 現代推薦          │ ❌ 不推薦用於 Slot         │ ✅ 推薦用於動態硬體           │
└───────────────────┴────────────────────────────┴─────────────────────────────┘
...
→ 開機就嘗試載入 driver，硬體不存在會報錯

【new_device 方式（entity-manager）】
// yosemite4.json
{
    "Exposes": [{
        "Name": "Slot1_Temp",
    ...
    "Probe": "xyz.openbmc_project.FruDevice({'BUS': 2})"
}
→ runtime 掃描 I2C，偵測到才載入 driver
→ 底層執行：echo "tmp75 0x48" > /sys/bus/i2c/devices/i2c-2/new_device
```
...
phosphor-hwmon = 被動讀取者
   → Kernel 透過 Device Tree 在「編譯時」就知道硬體位置
   → phosphor-hwmon 只是讀取 sysfs，不做探測

dbus-sensors + entity-manager = 主動探測者
   → entity-manager 在「運行時」掃描 I2C Bus
   → 只有偵測到硬體存在，才會實例化 Driver
```
...
【編譯時/開機前】
  ...
  Device Tree (.dts)              設定檔
  ┌─────────────────┐             ┌──────────────────────────────────┐
  │ i2c2 {          │             │ /etc/default/obmc/hwmon/2-0048.conf│
  │   sensor@48 {   │             │ LABEL_temp1=CPU0_Temp              │
  │     compatible  │             │ WARNHI_temp1=85000                 │
  │     = "ti,tmp75"│             └──────────────────────────────────┘
  │   }             │                          │
  └────────┬────────┘                          │
           │ 編譯到 Kernel                      │
...
【開機時】                                      │
                                               │
  Kernel 自動載入 tmp75 driver                 │
           │                                   │
           ▼                                   │
  /sys/class/hwmon/hwmon0/                     │
  ├── temp1_input  (讀取 = 45250)              │
  ├── temp1_max                                │
  └── name         (= tmp75)                   │
           │                                   │
           │                                   │
【Runtime】                                    │
  ...
  ┌────────────────────────────────────────────────┐
  │            phosphor-hwmon                       │
  │  1. 讀取設定檔，知道 2-0048 要對應到哪個 hwmon │
  │  2. 定時讀取 /sys/class/hwmon/hwmon0/temp1_input│
  │  3. 發布到 D-Bus                               │
  └────────────────────────────────────────────────┘
           │
           ▼
...
💡 關鍵點：
   - Kernel 透過 Device Tree 在「編譯時」就知道硬體位置
   - phosphor-hwmon 只是「被動讀取」sysfs，不做硬體探測
   - 如果 Slot 是空的，Device Tree 還是會嘗試載入 driver → 可能報錯
```

#### dbus-sensors + entity-manager 資料流

```
【Runtime - 動態偵測】

  JSON 設定檔 (描述「可能」存在的硬體)
  ┌────────────────────────────────────────────────────────────┐
  │ /usr/share/entity-manager/configurations/Yosemite4.json   │
  │                                                            │
  │ {                                                          │
  │   "Exposes": [{                                            │
  │     "Name": "Slot1 GPU Temp",                              │
  ...
  │     "Address": "0x48"                                      │
  │   }],                                                      │
  │   "Probe": "xyz.openbmc_project.FruDevice({'BUS': 2})"    │  ← 條件
  │ }                                                          │
  └────────────────────────────────────────────────────────────┘
           │
           ▼
  ...
  │                                                            │
  │  Step 1: 執行 Probe                                         │
  │          → 用 i2cdetect 掃描 Bus 2                          │
  │          → 0x48 有回應嗎？                                  │
  │                                                            │
  │  Step 2: 如果有回應 → 實例化 Driver                         │
  │          → echo "tmp75 0x48" > /sys/bus/i2c/.../new_device │
  │          → Kernel 動態載入 tmp75 driver！                   │
  │                                                            │
  │  Step 3: 在 D-Bus 發布 Inventory                            │
  ...
  └────────────────────────────────────────────────────────────┘
           ...
           ▼
  ┌────────────────────────────────────────────────────────────┐
  │            hwmon-temp-sensor (dbus-sensors 的一員)          │
  │                                                            │
  │  1. 收到 InterfacesAdded signal                            │
  │  2. 檢查 Type = TMP75 → 是我負責的！                        │
  │  3. 讀取 /sys/class/hwmon/.../temp1_input                  │
  │  4. 發布到 D-Bus                                           │
  └────────────────────────────────────────────────────────────┘
           │
           ▼
  D-Bus: /xyz/openbmc_project/sensors/temperature/Slot1_GPU_Temp

💡 關鍵點：
   - entity-manager 在「運行時」主動掃描 I2C Bus
   - 只有偵測到硬體存在，才會實例化 Driver
   - 如果 Slot 是空的，完全不會有錯誤
   - JSON 設定集中管理，不需要修改 Device Tree
```

### 🔥 實際差異對比：Slot 插拔情境

```
情境：Yosemite 4 有 4 個 Slot，Slot 2 沒有插卡

╔═══════════════════════════════════════════════════════════════════════════╗
║                   phosphor-hwmon 的問題                                   ║
╠═══════════════════════════════════════════════════════════════════════════╣
║                                                                           ║
║  Device Tree 定義了 4 個 Slot 的 Sensor：                                 ║
║  - Slot1 Sensor @ I2C-2 0x48  → ✅ 正常                                  ║
║  - Slot2 Sensor @ I2C-3 0x48  → ❌ 偵測失敗，Kernel dmesg 報錯           ║
║  - Slot3 Sensor @ I2C-4 0x48  → ✅ 正常                                  ║
║  - Slot4 Sensor @ I2C-5 0x48  → ✅ 正常                                  ║
║                                                                           ║
║  dmesg 錯誤訊息：                                                        ║
║  [   15.234567] tmp75 3-0048: Failed to read device id                   ║
║  [   15.234890] tmp75 3-0048: probe failed                               ║
║                                                                           ║
║  解決方法：要用多個不同的 .dts 檔案，或複雜的 Device Tree Overlay        ║
║                                                                           ║
╚═══════════════════════════════════════════════════════════════════════════╝

╔═══════════════════════════════════════════════════════════════════════════╗
║              dbus-sensors + entity-manager 的優雅處理                     ║
╠═══════════════════════════════════════════════════════════════════════════╣
║                                                                           ║
║  JSON 設定定義了 4 個「可能」的 Sensor：                                  ║
║                                                                           ║
║  entity-manager 啟動時：                                                  ║
║  - 掃描 I2C-2 @ 0x48 → 有回應 → 實例化 Driver → ✅ Sensor 出現           ║
║  - 掃描 I2C-3 @ 0x48 → 無回應 → 跳過 → 🔇 沒有錯誤，這是正常的           ║
║  - 掃描 I2C-4 @ 0x48 → 有回應 → 實例化 Driver → ✅ Sensor 出現           ║
║  - 掃描 I2C-5 @ 0x48 → 有回應 → 實例化 Driver → ✅ Sensor 出現           ║
║                                                                           ║
║  熱插拔 Slot 2 後：                                                       ║
║  - entity-manager 收到 FRU 偵測事件                                      ║
║  - 重新掃描 I2C-3 @ 0x48 → 有回應 → 實例化 Driver                        ║
║  - hwmon-temp-sensor 收到 signal → 新增 Sensor                           ║
║  - D-Bus 和 Redfish 自動出現新 Sensor！                                   ║
║                                                                           ║
╚═══════════════════════════════════════════════════════════════════════════╝
```
...
```bash
# ============== phosphor-hwmon Debug ==============

# 1. 確認 sysfs 有沒有建立
ls /sys/class/hwmon/

# 2. 確認設定檔有沒有對應
ls /etc/default/obmc/hwmon/
cat /etc/default/obmc/hwmon/2-0048.conf

# 3. 確認服務狀態
systemctl status xyz.openbmc_project.Hwmon@* 
...
# 1. 確認 entity-manager Probe 結果
busctl tree xyz.openbmc_project.EntityManager
# 看有沒有你的設備的 Inventory

# 2. 確認哪些 sensor daemon 在跑
systemctl list-units --type=service | grep sensor

# 3. 看 entity-manager 日誌
journalctl -u xyz.openbmc_project.EntityManager -f

# 4. 看 sensor daemon 日誌
journalctl -u xyz.openbmc_project.hwmontempsensor -f

# 5. 確認 sysfs 是否被動態建立
ls /sys/bus/i2c/devices/2-0048/
# 如果 entity-manager 成功 probe，這裡會有 hwmon 資料夾
```
...
```
┌──────────────────┬────────────────────────┬─────────────────────────────────┐
│       方面       │    phosphor-hwmon      │ dbus-sensors + entity-manager   │
├──────────────────┼────────────────────────┼─────────────────────────────────┤
│ 硬體知識來源     │ Device Tree (編譯時)   │ JSON + 運行時 Probe             │
│ 核心動作         │ 被動讀取 sysfs         │ 主動探測 → 實例化 → 讀取        │
│ 空 Slot 處理     │ Kernel 報錯            │ 優雅跳過                        │
│ 熱插拔支援       │ 需要 DT Overlay        │ 原生支援                        │
│ 設定修改         │ 需要重新編譯韌體       │ 只需修改 JSON，重啟服務         │
└──────────────────┴────────────────────────┴─────────────────────────────────┘
```
...
1. entity-manager 啟動
   → 讀取 JSON 設定，執行 Probe (掃描 I2C 設備)
   → **Driver 互動**：若 Probe 成功，它會寫入 sysfs (`new_device`) 來**實例化 Kernel Driver**
     (例如：告知 Kernel 在 I2C-2 0x48 有顆 TMP75，Kernel 才會載入對應 driver)
...
⚠️ **架構驗證：PLDM Sensor 的正確歸屬**
經查證 OpenBMC 架構與 Yosemite 4 實作：
- **`dbus-sensors`**：主要設計用於 **Local Hardware** (直接透過 I2C/ADC 連接在 BMC 上的裝置)。雖然源碼中有 `PldmSensor`，但在現代架構 (特别是 Intel 以外的平台如 Meta Y4) 較少使用。
- **`pldmd`**：這是處理 PLDM 協定的核心服務。對於 **Remote Sensor** (擴充卡上的 Sensor)，`pldmd` 負責：
  1. 透過 MCTP 傳輸層發送請求。
  2. 解析回應的 PDR (PLDM Data Records)。
  3. **直接**在 D-Bus 上建立 Sensor 物件 (Interface: `xyz.openbmc_project.Sensor.Value`)。
  
這解釋了為何在 `busctl tree` 中，你的擴充卡 Sensor 是掛在 `xyz.openbmc_project.PLDM` 服務下，而不是 `dbus-sensors`。兩者在現代架構是分工的：**Local 走 dbus-sensors，Remote 走 pldmd。**

dbus-sensors 服務列表 (專注於 Local Sensors)：
┌─────────────────────────┬────────────────────────────────────┐
│ 服務名稱                │ 支援硬體 (本地端連接)               │
├─────────────────────────┼────────────────────────────────────┤
│ hwmon-temp-sensor       │ TMP75, TMP112 等 I2C 温度 IC        │
│ psu-sensor              │ PMBus 電源供應器                    │
│ adc-sensor              │ ADC 電壓偵測                        │
│ fan-sensor              │ 風扇轉速 (Tach)                     │
│ intrusion-sensor        │ 機殼防開啟偵測                      │
│ nvme-sensor             │ 本地 NVMe SSD 溫度                  │
└─────────────────────────┴────────────────────────────────────┘
...
2. bmcweb 將 image 交給 phosphor-bmc-code-mgmt
   (注意：此服務舊稱 phosphor-software-manager，已更名)
   GitHub: https://github.com/openbmc/phosphor-bmc-code-mgmt

3. phosphor-bmc-code-mgmt 驗證：
   - 簽章驗證
   - 版本相容性檢查
   - Image 完整性（Hash）
